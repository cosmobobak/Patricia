...
auto uci_to_internal(std::string uci) -> Move {
...
auto attacks_square(Position position, int sq,
                    int color) -> bool {  // Do we attack the square at position "sq"?
...
auto is_queen_promo(Move move) -> bool { return extract_promo(move) == 3; }

auto is_cap(Position& position, Move move) -> bool {
...
auto make_move(Position& position, Move move, ThreadInfo& thread_info,
               bool update_nnue) -> int {  // Perform a move on the board.
