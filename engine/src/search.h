...
auto material_eval(Position& position) -> int16_t {
...
auto total_mat(Position& position) -> int16_t {
    int m = (position.material_count[0] + position.material_count[1]) * 100 +
            (position.material_count[2] + position.material_count[3]) * 300 +
            (position.material_count[4] + position.material_count[5]) * 300 +
            (position.material_count[6] + position.material_count[7]) * 500 +
...
auto total_mat_color(Position& position, int color) -> int16_t {
...
auto sacrifice_scale(Position& position, ThreadInfo& thread_info, Move move) -> int {
...
constexpr std::array<float, 5> danger_values = {0.8, 2.2, 2, 3, 6};
constexpr std::array<float, 5> defense_values = {1, 1.1, 1.1, 1, 1.7};

auto in_danger_white(Position& position) -> float {
...
auto in_danger_black(Position& position) -> float {
...
auto eval(Position& position, ThreadInfo& thread_info, int alpha, int beta) -> int {
    int color = position.color;
...
auto is_draw(Position& position, ThreadInfo& thread_info,
             uint64_t hash) -> bool {  // Detects if the position is a draw.
...
auto qsearch(int alpha, int beta, Position& position,
             ThreadInfo& thread_info) -> int {  // Performs a quiescence search on the
                                                // given position.
...
auto search(int alpha, int beta, int depth, Position& position,
            ThreadInfo& thread_info) -> int {  // Performs an alpha-beta search.
